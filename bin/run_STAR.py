...
parser.add_argument('fastq', nargs='+', help='FASTQ input (or mapped PE SAM). Format: fastq1 [fastq2], or comma-separated lists for each if multiple FASTQs/mate.')
...
elif args.fastq[0].endswith('.sam'):
    cmd += f' --readFilesType SAM PE --readFilesCommand samtools view'
...
if not args.fastq[0].endswith('.sam'):
    cmd += f" --outSAMattrRGline {' '.join(args.outSAMattrRGline)}"
